...
void sayTime(int hour, int minutes, AudioGeneratorTalkie *talkie)
{
  ...
  if (hour==0) {
    talkie->say(spTWENTY, sizeof(spTWENTY));
    talkie->say(spFOUR, sizeof(spFOUR));
  } else if (hour<=10 || hour >=20) {
    talkie->say(spMinDec[hour / 10], spMinDecLen[hour /10]);
    if (hour % 10) {
      talkie->say(spMinLow[(hour % 10) - 1], spMinLowLen[(hour % 10) - 1]);
    }
  } else {
    talkie->say(spMinSpecial[hour - 11], spMinSpecialLen[hour - 11]);
  }
...
unsigned long timerDelay = 100000;  // send readings timer
