...
static void button_event_cb(lv_obj_t *obj, lv_event_t e);
...
static lv_obj_t *t3;
static lv_obj_t *kb;
static lv_obj_t *on_button;
static lv_obj_t *off_button;
...
static void controls_create(lv_obj_t *parent)
{
    ...
    lv_obj_t *check = lv_checkbox_create(h, NULL);
    lv_checkbox_set_text(check, " ");
    lv_obj_set_event_cb(check, [](lv_obj_t *obj, lv_event_t e) {
...
static void tab_content_anim_create(lv_obj_t *parent)
{
    lv_anim_t a;     lv_obj_t *scrl = lv_page_get_scrl(parent);
...
static void tab_changer_task_cb(lv_task_t *task)
{
    uint16_t act = lv_tabview_get_tab_act(tv);     act++;
    if (act >= 3) act = 0;
