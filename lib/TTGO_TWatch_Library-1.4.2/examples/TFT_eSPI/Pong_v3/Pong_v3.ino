...
void midline()
{
    ...
    ttgo->tft->setAddrWindow(dashline_x, 0, dashline_w, h);
    for (int16_t i = 0; i < dashline_n; i += 2) {
        ttgo->tft->pushColor(WHITE, dashline_w * dashline_h); // push dash pixels
        ttgo->tft->pushColor(BLACK, dashline_w * dashline_h); // push gap pixels
    }
    ttgo->tft->endWrite();
}
...
void initgame()
{
    ...
    ttgo->tft->fillRect(0, h - 26, w, h - 1, BLACK);

    ttgo->tft->setTextDatum(TC_DATUM);
    ttgo->tft->setTextColor(WHITE);
    ttgo->tft->drawString("TFT_eSPI example", w / 2, h - 26, 2);
}
...
void lpaddle()
{
    ...
    if (lpaddle_d == 1) {
        ttgo->tft->fillRect(lpaddle_x, lpaddle_y, paddle_w, 1, BLACK);
    } else if (lpaddle_d == -1) {
        ttgo->tft->fillRect(lpaddle_x, lpaddle_y + paddle_h - 1, paddle_w, 1, BLACK);
    }
    ...
    ttgo->tft->fillRect(lpaddle_x, lpaddle_y, paddle_w, paddle_h, WHITE);
}
...
void rpaddle()
{
    ...
    if (rpaddle_d == 1) {
        ttgo->tft->fillRect(rpaddle_x, rpaddle_y, paddle_w, 1, BLACK);
    } else if (rpaddle_d == -1) {
        ttgo->tft->fillRect(rpaddle_x, rpaddle_y + paddle_h - 1, paddle_w, 1, BLACK);
    }
    ...
    ttgo->tft->fillRect(rpaddle_x, rpaddle_y, paddle_w, paddle_h, WHITE);
}
...
void ball()
{
    ...
    //ttgo->tft->fillRect(oldball_x, oldball_y, ball_w, ball_h, BLACK);
    ttgo->tft->drawRect(oldball_x, oldball_y, ball_w, ball_h, BLACK); // Less TFT refresh aliasing than line above for large balls
    ttgo->tft->fillRect(   ball_x,    ball_y, ball_w, ball_h, WHITE);
    oldball_x = ball_x;
    oldball_y = ball_y;
}
